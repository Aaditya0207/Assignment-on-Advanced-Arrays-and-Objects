...
const newarray = strings.filter(str => str.length >= 5);
...
const squarearray = numbers.map(num => Math.pow(parseInt(num), 2));
...
const newalphabets = alphabets.reduce((acc, current) => acc + current, '');
...
const index7 = ranks.findIndex(num => num === 7);
console.log(index7);
...
num.forEach((number, num2) => {
  console.log(`${num2}:${number * 2}`);
});
...
const positiveornot = positive.every(num => num > 0);
console.log(positiveornot);
...
cars.forEach((car, index) => {
  console.log(`${index + 1}. "${car.name}"${car.color ? ` ${car.color.join(' ')}` : ''}${car.capacity ? ` ${car.capacity}` : ''}`);
});
...
console.log(person.hasOwnProperty('bike')); // false
